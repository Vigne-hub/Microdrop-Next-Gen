from __future__ import annotations
from typing import Union, Sequence
import pika
from nptyping import NDArray, Shape, Float, UInt8

from refrac_qt_microdrop.helpers.dmf_utils import ElectrodeDict
from refrac_qt_microdrop.helpers.logger import initialize_logger
from refrac_qt_microdrop.helpers.pub_sub_manager import PubSubManager
from refrac_qt_microdrop.pydantic_models.electrode_signal_model import ElectrodeStateChanged, MetastateChanged, \
    StateChanged

logger = initialize_logger(__name__)


class Electrode:

    @classmethod
    def from_dict(cls, d: ElectrodeDict, parent=None):
        return cls(d['channel'], d['path'], parent=parent)

    def __init__(self, channel: int, path: NDArray[Shape['*, 1, 1'], Float], parent=None):
        super().__init__(parent=parent)
        self.channel = channel  #electrode id
        self.path = path  #NDArray path to electrode
        self._state = False
        self._metastate = None

        logger.debug("Electrode %s created", self.channel)  # on test loaded 93 electrodes

        self.pub_sub_manager = PubSubManager()
        self.pub_sub_manager.create_publisher(publisher_name=f'electrode_publisher_{self.channel}', exchange_name='state_changed')
        self.pub_sub_manager.create_publisher(publisher_name=f'electrode_publisher_{self.channel}', exchange_name='metastate_changed')

    def emit_signal(self, message: Union[ElectrodeStateChanged, MetastateChanged, StateChanged]):
        self.pub_sub_manager.publish(message=message, publisher=f'electrode_publisher_{self.channel}')
        logger.info(f"Emitted signal: {message}")

    def get_state(self) -> bool:
        return self._state

    def set_state(self, state: bool):
        if state != self._state and self.channel is not None:  # Only emit signal if state has changed
            self._state = state
            self.emit_signal(StateChanged(Signal='state_changed', Channel=self.channel, State=self._state))
            logger.debug("State changed to %s for %s", self.get_state(), self.channel)

    def get_metastate(self) -> object:
        return self._metastate

    def set_metastate(self, metastate: object):
        if metastate != self._metastate:
            self._metastate = metastate
            self.emit_signal(StateChanged(Signal='metastate_changed', Channel=self.channel, State=self.get_metastate()))

    def toggle_state(self):
        logger.debug("State changed to %s for %s", not self.get_state(), self.channel)
        self.set_state(not self.get_state())  # Correct way to toggle state


class Electrodes:

    def __init__(self, electrodes: dict[str, Electrode] = None, parent=None):
        super().__init__(parent=parent)
        if electrodes:
            self.electrodes = electrodes
        else:
            self.electrodes: dict[str, Electrode] = {}

        self.pub_sub_manager = PubSubManager()
        self.pub_sub_manager.create_subscriber(subscriber_name='electrodes_subscriber')
        self.pub_sub_manager.bind_sub_to_pub(subscriber='electrodes_subscriber', exchange_name='state_changed')
        self.pub_sub_manager.start_consumer(subscriber='electrodes_subscriber', func=self.consume_signal)

        self.pub_sub_manager.create_publisher(publisher_name='electrodes_publisher', exchange_name='electrode_state_changed')

    def consume_signal(self, ch, method, properties, body):
        try:
            message = StateChanged.model_validate_json(body)
            if message.Signal == 'state_changed':
                self.emit_signal(ElectrodeStateChanged(Signal='electrode_state_changed', Channel=message.Channel,
                                                       State=message.State))
                logger.info(
                    f"Received signal: {message}, emitted signal: "
                    f"{ElectrodeStateChanged(Signal='electrode_state_changed', Channel=message.Channel, State=message.State)}")
        except Exception as e:
            logger.error(f"Failed to process message: {e}")

    def emit_signal(self, message: Union[ElectrodeStateChanged, MetastateChanged, StateChanged]):
        self.pub_sub_manager.publish(message=message, publisher='electrode_publisher')
        logger.info(f"Emitted signal: {message}")

    def __getitem__(self, item: str) -> Electrode:
        return self.electrodes[item]

    def __setitem__(self, key, value):
        self.electrodes[key] = value

    def values(self):
        return self.electrodes.values()

    def keys(self):
        return self.electrodes.keys()

    def items(self):
        return self.electrodes.items()

    def set_electrodes(self, electrodes: dict[str, Electrode]):
        self.electrodes = electrodes

    def toggle_all_electrodes_off(self):
        for e in self.electrodes.values():
            e.set_state(False)

    def toggle_on_batch(self, electrodes):
        logger.debug("Attempt to toggling on batch electrodes: %s", electrodes)
        for e in self.electrodes.keys():
            if e in electrodes:
                self.electrodes[e].set_state(True)

    def sync_electrode_states(self, states: Sequence[str | int]):
        for k, v in self.electrodes.items():
            try:
                v.state = states[v.channel]
            except KeyError:
                logger.warning(f"Channel {v.channel} not found in states")

    def sync_electrode_metastates(self, metastates: Sequence[str | int]):
        for k, v in self.electrodes.items():
            try:
                v.metastate = metastates[v.channel]
            except KeyError:
                logger.warning(f"Channel {v.channel} not found in metastates")

    def check_electrode_range(self, n_channels: int) -> list[str]:
        """
        Checks that the electrode channel numbers are within n_channels
        :param n_channels: number of channels present
        :return: list of electrode names that are out of range
        """
        return [k for k, v in self.electrodes.items() if v.channel >= n_channels]
