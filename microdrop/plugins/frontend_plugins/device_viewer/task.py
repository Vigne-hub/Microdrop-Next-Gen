...
DEFAULT_SVG_FILE = os.path.dirname(__file__) + f"{os.sep}2x3device.svg"
...
class DeviceViewerTask(Task):
    ...
    def on_electrode_clicked(_model: Electrode, _electrode_view: ElectrodeView):
        ...
        publish_message(topic="device_view/signal/electrode_clicked",
                        message=f"Electrode channel: {_model.channel}, State: {_model.state}")

        # Do some other notification or updates or action here...

    def update_view(self, message):
        """Update the view based on the received message."""
        logger.debug(f"Updating view with message: {message}")

        for electrode in self.electrodes_model.electrodes.values():
            electrode.state = False  # clear

        electrodes_to_turn_on = message.split(",") # channel numbers
        for electrode in self.electrodes_model.electrodes.values():
            if electrode.channel in electrodes_to_turn_on:
    ...
    def activated(self):
        ...
        self.device_view_listener = self.create_device_view_listener_actor()

    def create_device_view_listener_actor(self):
        """
        ...
        @dramatiq.actor
        def device_view_listener(message, topic):
            """
            ...
            if topic[-1] == "turn_electrodes_on":
                self.update_view(json.loads(message))

        return device_view_listener
