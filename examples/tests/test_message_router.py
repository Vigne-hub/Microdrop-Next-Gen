...
class TestMessageRouterActor:
    ...
    def test_message_router_actor_can_route_message(self, router_actor, stub_broker):
        ...
        with worker(stub_broker, worker_timeout=1000):
            stub_broker.join("default")

        # I expect the database to be populated
        assert database == {test_topic: test_message}

    def test_message_router_actor_can_route_message_to_multiple_subscribing_actors(self, router_actor, stub_broker):
        from microdrop_utils.dramatiq_pub_sub_helpers import publish_message
